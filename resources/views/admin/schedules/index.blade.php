...
.schedule-table .holiday { background: #f8f9fa !important; color: #adb5bd; }
.schedule-table .schedule-clickable { cursor: pointer; border-radius: 3px; padding: 2px 0; }
.schedule-table .schedule-clickable .badge { font-size: .65rem; }
.schedule-table .schedule-clickable:hover:not(.holiday) { background: #e9ecef; }
...
document.getElementById('shiftForm').addEventListener('submit', function(e) {
    ...
    fetch('/admin/schedules/user/' + userId + '/update', {
        method: 'POST',
    ...
    .then(data => {
        if (data.success) {
            const cell = document.querySelector(`td[data-user-id="${userId}"][data-date="${tanggal}"]`);
            if (cell) {
                if (data.shift_id) {
